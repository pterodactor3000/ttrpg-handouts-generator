...
|---|---|
| `SUPABASE_URL` | `supabase status -o env` (`SUPABASE_URL` key) |
...
**`supabase status -o env` variable name mapping.** The CLI outputs
`ANON_KEY` and `SERVICE_ROLE_KEY`, but `src/integration/setup-env.ts` loads
`.env.test` and the helpers read `SUPABASE_ANON_KEY` and
`SUPABASE_SERVICE_ROLE_KEY`. The `.env.test` write step must grep each key from
the status output and rename it. Verify the exact CLI output locally with
`npx supabase status -o env` before writing the workflow; if the key names differ
from expectations, adjust the grep patterns. The `SUPABASE_URL` key name matches
directly between CLI output and `.env.test`.
