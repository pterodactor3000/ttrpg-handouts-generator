...
- Active cards use **Archive** (soft-delete). Archived cards use **Delete** (permanent row removal via `DELETE /api/handouts/[id]` + RLS `gm_delete_archived`).
...
- No schema, RLS, or migration changes for archive alone — F-01 already provides archive transition. Permanent delete adds migration `20260618200000_gm_delete_archived_handouts.sql`.
