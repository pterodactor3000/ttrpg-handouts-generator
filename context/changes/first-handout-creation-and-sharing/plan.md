...
**Intent**: Server-rendered shell that mounts the `HandoutEditor` React island. Reads `Astro.locals.user` to confirm auth (middleware already enforces this, but the GM's user ID is passed as a prop to the island so the API route doesn't need to re-identify the user from the session — Supabase RLS handles that server-side).
...
**Contract**: Component state: `title: string`, `markdownContent: string`, `backgroundCategory: BackgroundCategory | null`, `tags: string[]`, `handoutId: string | null` (null until first save), `savedHandoutId: string | null`, `isSaving: boolean`, `shareToken: string | null` (set after publish), `shareDialogOpen: boolean`. Layout: two-column on `md+` screens (form column left, preview column right), single column stacked on mobile. The preview column renders a `<div>` with the selected background's `cssBackground` applied as inline `style.background` and `style.backgroundSize = 'cover'`; a semi-transparent panel inside holds the rendered HTML from `renderHandoutHtml(markdownContent)` set via `dangerouslySetInnerHTML`. Save calls `POST /api/handouts` (creates) or `PUT /api/handouts/[id]` (updates if `handoutId` is set); Share is disabled until `handoutId` is set (i.e. at least one draft save has been done). The Share button calls `POST /api/handouts/[id]/publish`.
