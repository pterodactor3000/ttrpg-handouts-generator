...
This product provides a focused handout manager: GMs create handouts with markdown support against a background image, preview them, and share them via permanent links. Players access handouts via read-only links. The handout is the source of truth, not scattered notes.
...
- Organization by tags so handouts don't pile into an unsearchable list

**Secondary persona**: Players who receive handout links and need to reference them later without losing them. Players access handouts via read-only shareable links (no login required).
...
The app moves handouts through draft → published → archived states, transitioning to published when a share link is generated and to archived when the handout ages past 365 days or is manually soft-deleted.
...
_None. All critical decisions were resolved during shaping. Quality check status: accepted._
